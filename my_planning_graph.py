...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            # create PgNode_a object for given action
            ...
            if node.prenodes.issubset(self.s_levels[level]):
                # connect to the S node instances
                node_holder.append(node)
                # for each node at the current level update children and parent conditions
                for n in self.s_levels[level]:
                    n.children.add(node)
                    node.parents.add(n)
        # append to a_levels
        self.a_levels.append(node_holder)
    ...
    def add_literal_level(self, level):
        ...
        node_holder = []
        # For each node (except lass one)
        for node in self.a_levels[level-1]:
            # for S nodes in effnodes
            for s_node in node.effnodes:
                # add s_node to the holder
                node_holder.append(s_node)
                # update children and parent conditions
                s_node.parents.add(node)
                node.children.add(s_node)

        # append only distinct elements
        self.s_levels.append(list(set(node_holder)))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if a1 condition is cancelled in a2
        a1_cancelled = bool(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem))
        # check if a2 condition is cancelled in a1
        a2_cancelled = bool(set(node_a2.action.effect_add) & set(node_a1.action.effect_rem))

        # return True if at least one condition is met
        return a1_cancelled | a2_cancelled
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if effect is the negation of a precondition of the other
        a1_add_conflict = bool(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg))
        a2_add_conflict = bool(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg))
        a1_rem_conflict = bool(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos))
        a2_rem_conflict = bool(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos))

        # return True if at least one of the above conditions is met
        return a1_add_conflict | a2_add_conflict | a1_rem_conflict | a2_rem_conflict
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # iterate through all parent pairs
        for a1_parent in node_a1.parents:
            for a2_parent in node_a2.parents:
                # if there is at least one mutex pair set state to true
                state |= a1_parent.is_mutex(a2_parent)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        symbol_match = node_s1.symbol == node_s2.symbol
        # check if pos values are different
        pos_diff = node_s1.is_pos != node_s2.is_pos

        # return true if both conditions are met
        return symbol_match and pos_diff
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_parent in node_s1.parents:
            for s2_parent in node_s2.parents:
                # if there is pair which is not mutex then set state to false
                state &= s1_parent.is_mutex(s2_parent)
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            # iterate through all s level nodes
            for level, nodes in enumerate(self.s_levels):
                # check if goal condition is in the nodes
                if PgNode_s(goal, 1) in nodes:
                    # update level if condition is met and go to next level
                    level_sum += level
                    break
